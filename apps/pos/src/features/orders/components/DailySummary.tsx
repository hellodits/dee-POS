    ...
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="text-center mb-6 pb-6 border-b border-border">
          ...
          <p className="text-3xl sm:text-4xl font-bold text-foreground">{formatCurrency(totalRevenue)}</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
            ...
            <p className="text-xl font-bold text-green-700 dark:text-green-300">{completedOrders.length}</p>
            <p className="text-xs text-green-600 dark:text-green-400">Selesai</p>
          </div>
          <div className="text-center p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
            <Clock className="w-5 h-5 text-yellow-600 dark:text-yellow-400 mx-auto mb-1" />
            <p className="text-xl font-bold text-yellow-700 dark:text-yellow-300">{inProcessOrders.length}</p>
            <p className="text-xs text-yellow-600 dark:text-yellow-400">Proses</p>
          </div>
          <div className="text-center p-3 bg-muted rounded-lg">
            <ShoppingBag className="w-5 h-5 text-muted-foreground mx-auto mb-1" />
            <p className="text-xl font-bold text-foreground">{cancelledOrders.length}</p>
        ...
        <div className="space-y-3 mb-6">
          <h4 className="text-sm font-semibold text-foreground">Rincian Pendapatan</h4>
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Subtotal</span>
              <span className="font-medium text-foreground">{formatCurrency(totalSubtotal)}</span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            {totalTip > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-foreground">{formatCurrency(totalTip)}</span>
              </div>
            )}
        ...
        {completedOrders.length > 0 && (
          <div className="space-y-3 mb-6">
            <h4 className="text-sm font-semibold text-foreground">Metode Pembayaran</h4>
            <div className="space-y-2">
              <div className="flex items-center justify-between p-2 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <span className="text-sm font-medium text-green-700 dark:text-green-300">{formatCurrency(paymentBreakdown.cash)}</span>
              </div>
              <div className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div className="flex items-center gap-2">
                  <CreditCard className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                  <span className="text-sm text-foreground">Card/QRIS</span>
                </div>
                <span className="text-sm font-medium text-blue-700 dark:text-blue-300">{formatCurrency(paymentBreakdown.card)}</span>
              </div>
              <div className="flex items-center justify-between p-2 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                <div className="flex items-center gap-2">
                  <Smartphone className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                  <span className="text-sm text-foreground">E-Wallet</span>
                </div>
                <span className="text-sm font-medium text-purple-700 dark:text-purple-300">{formatCurrency(paymentBreakdown.ewallet)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-muted rounded-lg p-4 text-center">
          <p className="text-xs text-muted-foreground mb-1">Rata-rata per Order</p>
